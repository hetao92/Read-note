...
- `document.documentElement.offsetHeight`：包括内边距、滚动条、边框, 不包括外边距。
...
3. 将 DOM 与 CSSOM 合并成一个渲染树。
   ...
   **渲染树只会包括需要显示的节点和这些节点的样式信息**，会把DOM树中没有的元素给去除，比如head标签以及里面的内容，以及display:none的元素也会被去除，但是 visibility 属性值为“hidden”的元素仍会显示。

4. Layout：根据渲染树来布局（回流），计算每个节点的坐标和大小，是否换行，position 等属性。

5. 调用 GPU 绘制，这个过程是在多个层上完成的

6. 合成图层，显示在屏幕上。



在词法分析的时候，会将内容分解成有效标记，解析器依据语言的词法规则分析文档的结构来构建解析树，并且词法分析器会将无关的字符分离出来，比如空格换行符等。解析器在分析时候会向词法分析器请求一个标记，并将其与某条语法规则进行匹配，找到相应规则后会将对应于该标记的节点添加到解析树中，并请求下一个标记。若没有规则匹配，则将标记存储到内部，并继续请求标记，直到找到可与所有内部存储的标记匹配的规则

解析器有两种：自上而下解析器（从语法的高层结构出发，尝试从中找到匹配的规则）；自下而上解析器（从低层规则出发，将输入内容逐步转化为语法规则，直至满足高层规则。

**DTD**

HTML 无法用上述的两种解析方法，因为它的语法不是与上下文无关的语法，HTML 是和上下文关系紧密的，因此需要具有一定的容错机性，就采用了 DTD 格式
...
如果图层中某个元素需要重绘，那么整个图层都需要重绘。比如一个图层包含很多节点，其中有个gif图，gif图的每一帧，都会重绘图层的其他节点，然后生成最终的图层位图。所以这需要通过特殊的方式来强制gif图属于自己一个图层 (如translateZ(0)`或者`translate3d(0,0,0))
