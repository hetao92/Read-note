...
DOM是复杂的，对它的操作是非常耗资源的，仅一个 div，他的实例属性就多达200多个。因此如果用更轻量级的JS对象来代替复杂DOM节点，然后把对DOM的diff操作转移到JS对象，就可以避免大量对DOM的查询操作

React 将传统的命令式直接操作DOM（代码可读性差，可维护性低）变成了声明式，摈弃直接操作DOM的细节，只关注数据的变动，DOM由框架完成，从而提升代码可读性与可维护性

需要注意的是，虚拟DOM和Diff算法的出现是为了解决由命令式编程转变为声明式编程、数据驱动后带来的性能问题，因为diff本身也是十分消耗性能的，直接操作DOM的性能可能效率上更高
